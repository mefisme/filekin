# HANDOFF-ARCHIVE.md — Filekin

**Frozen history. Do not act on anything in this file.**

This is `HANDOFF.md` as it stood on 2026-08-28, before it was cut back to live state only. It is kept
because it holds the reasoning behind decisions that are now only summarised, the per-session
implementation records, the PowerShell/ConPTY spike findings, and the reference sources. Some of it is
stale by design: the "Immediate Next Task", "Recommended Next Step", and "Specified but Not
Implemented" sections below describe the project on earlier dates and have since been overtaken.

**`HANDOFF.md` is the only authoritative statement of the current phase, the next task, and what is
blocked.** Read this file only when you need the history behind something `HANDOFF.md` states as a
conclusion.

Nothing new should be appended here except a completed-work record moved out of `HANDOFF.md`.
