...
- **The two real-Recycle-Bin integration tests cannot run on a hosted CI runner.** `WindowsRecycleBin` reads the bin through `Shell.Application`, which needs an interactive Windows shell; a GitHub-hosted runner has none, so enumeration returns nothing. They failed on their first CI run (`33008547374`) for that reason, not because of a code defect. They now call `RequireObservableRecycleBin`, which marks the test **inconclusive** only when the bin lists as completely empty immediately after recycling a file — impossible on a working desktop, so a populated-but-wrong bin still fails properly. Real coverage therefore comes from desktop runs; keep verifying these locally before trusting a green CI.
