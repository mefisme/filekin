...
1. The **`@` reference resolver** (`ARCHITECTURE.md` §3) — resolve workspace references (current folder, selection, user Locations) to concrete paths before shell/terminal dispatch. The `@`-vs-native-PowerShell-`@` disambiguation rule is decided (2026-08-25 — known command-bar references win; see Product Questions below), so this is now unblocked.
2. The **application-command (`/`) dispatch** subsystem that consumes `CommandRouterResult.AppCommandInput` and runs built-in commands. **Done (2026-08-25):** `Filekin.Core.Commands.App` + the four core file-operation commands over `IFileSystemOperations`, with `WindowsFileSystemOperations`. See Work Completed and Recommended Next Step.
...
The `/` application-command dispatch is implemented: `Filekin.Core.Commands.App` (quote-aware `AppCommandParser`, `AppCommandDispatcher`, `AppCommandContext`/`AppCommandResult`, `IAppCommand`, `BuiltInAppCommands`) and the four core file-operation commands (`/copy`, `/move`, `/rename`, `/delete`) over `IFileSystemOperations`, with `WindowsFileSystemOperations` (System.IO copy/move; Recycle Bin delete via `SHFileOperationW`). The remaining non-UI core piece is the **`@` reference resolver** (`ARCHITECTURE.md` §3), now **unblocked** — the `@`-vs-PowerShell-`@` disambiguation was decided 2026-08-25 (known command-bar references win; splatting lives in terminal tabs — see DECISIONS.md "Known Command-Bar References Win Over PowerShell Splatting"). The resolver expands `@thisfolder` / `@selection` / user-Location tokens into concrete paths before dispatch; the file-operation commands already consume plain resolved paths and multi-`@selection` batches map to repeated invocations. After the resolver, begin the WPF surface: a VT-interpreting terminal renderer over `ConPtyTerminalSession.OutputReceived`, the Files hierarchy, and the command bar wired to `CommandRouter` + `AppCommandDispatcher`.
