...
Terminal follow-ups that are deliberately **not** implemented and need a product decision before anyone builds them: keyboard tab switching (no Ctrl+Tab / Ctrl+1..9 binding exists yet), terminal mouse reporting, mouse text selection and copy, and full screen-reader text exposure for the terminal surface. See **Known Problems**.
...
- **Terminal tabs can only be switched with the mouse.** There is no keyboard binding (Ctrl+Tab, Ctrl+1..9, or otherwise) for moving between the Files tab and terminal tabs. For a keyboard-first product this is a real gap, but the binding is not specified in `UX-DESIGN.md`, so it needs a product decision rather than an invented shortcut.
- **Terminal mouse reporting, mouse text selection/copy, and full screen-reader text exposure are not implemented.** `TerminalControl` has only a basic automation peer (`Document` control type with a name and help text); the cell grid is not exposed as text to assistive technology. TUIs that request mouse tracking (`?1000/1002/1003/1006`) simply do not receive mouse events.
...
1. Ask the owner for the two terminal decisions recorded under **Product Questions Requiring Owner Decision**: the keyboard binding for tab switching, and whether terminal mouse selection/copy plus assistive-text exposure are in v1 scope.
...
- **Keyboard binding for workspace/tab switching — open.** Terminal tabs currently switch by mouse only. Filekin is keyboard-first, but `UX-DESIGN.md` does not specify a binding, and the obvious candidates collide: `Ctrl+Tab` and `Ctrl+1..9` are both plausible, and any binding must not be swallowed before the hosted shell sees it while a terminal is focused (the window-level handler deliberately stops intercepting keys once a terminal is selected). Decide the binding and how it escapes terminal input.
- **Terminal mouse selection/copy, mouse reporting, and assistive-text exposure in v1? — open.** None are implemented. Mouse text selection and copy is the one users notice first; terminal mouse reporting (`?1000/1002/1003/1006`) is what TUIs ask for; exposing the cell grid as text to screen readers is the accessibility item. Each is real work and none is specified.
